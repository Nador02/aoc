...
def main():
    with open("input.txt", "r") as f:
        ...
        for i in range(len(schematic)):
            ...
            symbol_positions = []
            if i != 0:
                symbol_positions.extend([m.start() for m in re.finditer(_SYMBOL_PATTERN, schematic[i-1])])
            if i != len(schematic)-1:
                symbol_positions.extend([m.start() for m in re.finditer(_SYMBOL_PATTERN, schematic[i+1])])
            symbol_positions.extend([m.start() for m in re.finditer(_SYMBOL_PATTERN, schematic[i])])
                
            for part_number, positions in part_numbers:
...
def check_if_symbol_near_part_number(positions, symbol_positions):
    positional_range = range(positions[0]-1,positions[1]+1)
